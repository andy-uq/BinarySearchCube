...
inline void *find_key(struct cube *cube, int key, short *w, short *x, short *y, short *z);
...
void *find_index(struct cube *cube, int index, short *w_index, short *x_index, short *y_index, short *z_index);
...
/*					for (z = x_node->z_size[y] - 1 ; z >= 0 ; z--)
					{
						free(y_node->z_vals[z]);
					}
*/
					free(y_node);
				}
...
void set_key(struct cube *cube, int key, void *val)
{
	...
	if (cube->w_size == 0)
	{
		...
		cube->x_size = (short *) malloc(BSC_M * sizeof(short));
		...
		w_node->y_size = (short *) malloc(BSC_M * sizeof(short));
		w_node->x_volume = (short *) malloc(BSC_M * sizeof(short));
...
inline void insert_w_node(struct cube *cube, short w)
{
	...
	if (cube->w_size == cube->m_size)
	{
		...
		cube->x_size = (short *) realloc(cube->x_size, cube->m_size * sizeof(short));
	}
	...
	if (w + 1 != cube->w_size)
	{
		...
		memmove(&cube->x_size[w + 1], &cube->x_size[w], (cube->w_size - w - 1) * sizeof(short));
	}
	...
	w_node->y_size = (short *) malloc(cube->m_size * sizeof(short));
	w_node->x_volume = (short *) malloc(cube->m_size * sizeof(short));
}
...
void remove_w_node(struct cube *cube, short w)
{
	...
	if (cube->w_size)
	{
		if (cube->w_size != w)
		{
			...
			memmove(&cube->x_size[w], &cube->x_size[w + 1], (cube->w_size - w) * sizeof(short));
		}
	}
	else
	{
...
inline void insert_x_node(struct cube *cube, short w, short x)
{
	...
	if (x_size % BSC_M == 0 && x_size < cube->m_size)
	{
		...
		w_node->x_volume = (short *) realloc(w_node->x_volume, cube->m_size * sizeof(short));
		w_node->y_size = (short *) realloc(w_node->y_size, cube->m_size * sizeof(short));
	}
	...
	if (x_size != x + 1)
	{
		...
		memmove(&w_node->y_size[x + 1], &w_node->y_size[x], (x_size - x - 1) * sizeof(short));
	}
...
void remove_x_node(struct cube *cube, short w, short x)
{
	...
	if (cube->x_size[w])
	{
		if (cube->x_size[w] != x)
		{
			...
			memmove(&w_node->y_size[x], &w_node->y_size[x + 1], (cube->x_size[w] - x ) * sizeof(short));
		}
...
void split_w_node(struct cube *cube, short w)
{
	...
	memcpy(&w_node2->y_size[0], &w_node1->y_size[cube->x_size[w]], cube->x_size[w + 1] * sizeof(short));
...
void merge_w_node(struct cube *cube, short w1, short w2)
{
	...
	w_node1->x_volume = (short *) realloc(w_node1->x_volume, cube->m_size * sizeof(short));
	w_node1->y_size = (short *) realloc(w_node1->y_size, cube->m_size * sizeof(short));
	...
	memcpy(&w_node1->y_size[cube->x_size[w1]], &w_node2->y_size[0], cube->x_size[w2] * sizeof(short));
